...
class TableData {
    ...
    public function checkField($val) {
        return ($val == "no" || $val == "") ? "" : "<i class='icon-ok'></i>";
    }

    public function toIcon($val) {
    ...
    public function get($table, $index_column, $columns) {
        ...
        foreach ($rResult as $aRow) {
            ...
            $edit .= "&nbsp;<button class='btn btn-small btn-warning' onclick='navColumns($id,\"$title\")' style='width:88px'><i class='icon-columns'></i> Columns</button>";
            $row[] = $edit;
            $output['aaData'][] = $row;
        }
