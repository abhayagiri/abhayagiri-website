...
m.each(this.split(","),function(ra,sa){if(sa=="title"){fa.append("<span class='fc-header-title'><h3>&nbsp;</h3></span>");ga&&ga.addClass(q+"-corner-right");ga=null}else{var ha;if(a[sa])ha=a[sa];else if(Ja[sa])ha=function(){na.removeClass(q+"-state-hover");a.changeView(sa)};if(ha){ra=b.theme?jb(b.buttonIcons,sa):null;var da=jb(b.buttonText,sa),na=m("<span class='btn'><span class='fc-button-inner'><span class='fc-button-content'>"+(ra?"<span class='fc-icon-wrap'><span class='ui-icon ui-icon-"+
...
ga||na.addClass(q+"-corner-left");ga=na}}}});ga&&ga.addClass(q+"-corner-right")});return fa}function g(u){Q.find("h3").html(u)}function l(u){Q.find("span.fc-button-"+u).addClass(q+"-state-active")}function j(u){Q.find("span.fc-button-"+u).removeClass(q+"-state-active")}function t(u){Q.find("span.fc-button-"+u).addClass(q+"-state-disabled")}function y(u){Q.find("span.fc-button-"+u).removeClass(q+"-state-disabled")}var S=this;S.render=e;S.destroy=d;S.updateTitle=g;S.activateButton=l;S.deactivateButton=
...
I){var R,V=la+"-widget-header",ea=la+"-widget-content",aa;R="<table class='table table-striped table-bordered dataTable' style='width:100%' cellspacing='0'><thead><tr>";for(aa=0;aa<F;aa++)R+="<th class='fc- "+V+"'/>";R+="</tr></thead><tbody>";for(aa=0;aa<w;aa++){R+="<tr class='fc-week"+aa+"'>";for(V=0;V<F;V++)R+="<td class='fc- "+ea+" fc-day"+(aa*F+V)+"'><div>"+(I?"<div class='fc-day-number'/>":"")+"<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></div></td>";R+="</tr>"}R+="</tbody></table>";w=
...
var list_view;
(function(f){
    var d=f.fullCalendar;
    var c=d.formatDate;
    var g=d.parseISO8601;
    var a=d.addDays;
    var b=d.applyAll;
    d.sourceNormalizers.push(function(h){
        if(h.dataType=="gcal"||h.dataType===undefined&&(h.url||"").match(/^(http|https):\/\/www.google.com\/calendar\/feeds\//)){
            h.dataType="gcal";
            if(h.editable===undefined){
                h.editable=false
                }
            }
    });
d.sourceFetchers.push(function(j,k,h){
    if(j.dataType=="gcal"){
        return e(j,k,h)
        }
    });
function e(j,n,h){
    var l=j.success;
    var k=f.extend({},j.data||{},{
        "start-min":c(n,"u"),
        "start-max":c(h,"u"),
        singleevents:true,
        "max-results":9999
    });
    var m=j.currentTimezone;
    if(m){
        k.ctz=m=m.replace(" ","_")
        }
        return f.extend({},j,{
        url:j.url.replace(/\/basic$/,"/full")+"?alt=json-in-script&callback=?",
        dataType:"jsonp",
        data:k,
        startParam:false,
        endParam:false,
        success:function(r){
            var q=[];
            if(r.feed.entry){
                f.each(r.feed.entry,function(x,A){
                    var y=A["gd$when"][0]["startTime"];
                    var u=g(y,true);
                    var z=c(u,"MMMM dS, dddd");
                    var s=c(u,"yyyyMMdd");
                    var v=c(u,"hh:mm TT");
                    var w=g(A["gd$when"][0]["endTime"],true);
                    var B=y.indexOf("T")==-1;
                    var t;
                    f.each(A.link,function(C,D){
                        if(D.type=="text/html"){
                            t=D.href;
                            if(m){
                                t+=(t.indexOf("?")==-1?"?":"&")+"ctz="+m
        ...
        var o=[q].concat(Array.prototype.slice.call(arguments,1));
        var p=b(l,this,o);
        if(f.isArray(p)){
        return p
        }
        if(q.length==0){
        f("#notify").html("No events have been listed for this month yet")
        }else{
        f("#notify").html("")
        }
        abhaya_events=new Array();
        for(i=0;i<q.length;i++){
        abhaya_events[i]=q[i].stamp+"~";
        abhaya_events[i]+=q[i].when;
        abhaya_events[i]+='<br/><a href="#" class="title showdesc"> '+q[i].title+"</a>";
        abhaya_events[i]+='<div class="hidden-event left"><span>'+q[i].description+"</span></div><br/><br/>"
        }
        abhaya_events.sort();
        list_view="";
        for(i=0;i<abhaya_events.length;i++){
        list_view+='<div class="event">'+abhaya_events[i].split("~")[1]+"</div>"
        }
        f("#list-view").html(list_view);
        f(".showdesc").toggle(function(){
        f(this).parent().find("div.hidden-event").animate({
            height:f(this).parent().find("div.hidden-event").find("span").height()
            },500)
        },function(){
        f(this).parent().find("div.hidden-event").animate({
            height:"0"
