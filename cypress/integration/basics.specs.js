...
context('books', () => {
    ...
    it('see important information when requesting a book', () => {
        cy  .get('#header-menu-button').click()
            ...
            .get('[data-cy=request-book]').click()
    ...
    it('filters books by author, language and availability', () => {
        cy  .get('#header-menu-button').click()
            .get('[href="/books"]').click()
            .get('#main').find('article').should('have.length', 2)
            .get('[data-cy=author-select]').select('1')
            .get('[data-cy=filter-books]').click()
            .get('#main').find('article').should('have.length', 1)
            .get('[data-cy=author-select]').select('all')
            .get('[data-cy=filter-books]').click()
            .get('#main').find('article').should('have.length', 2)
            .get('[data-cy=language-select]').select('1')
            .get('[data-cy=filter-books]').click()
            .get('#main').find('article').should('have.length', 1)
            .get('[data-cy=language-select]').select('all')
            .get('[data-cy=filter-books]').click()
            .get('#main').find('article').should('have.length', 2)
            .get('[data-cy=request-select]').select('1')
            .get('[data-cy=filter-books]').click()
            .get('#main').find('article').should('have.length', 1)
            .get('[data-cy=request-select]').select('all')
            .get('[data-cy=filter-books]').click()
            .get('#main').find('article').should('have.length', 2)
    })
})
